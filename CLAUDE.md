...
which runs, with `nvm use` (24.14):

```bash
( cd packages/jbrowse-plugin-apollo     && JB_UMD=true yarn build )   # @apollo-annotation/shared + tsc project-ref deps + JBrowse UMD plugin
( cd packages/apollo-collaboration-server && yarn build )             # build:shared + nest build (schemas/common/mst/shared dists + server)
```
...
- **Use the pixi `test` task, not bare jest.** It invokes
  `yarn node --experimental-vm-modules $(yarn bin jest)`; a bare
  `NODE_OPTIONS=... jest` breaks the Yarn-PnP wrapper.
...
- Node is pinned to **24.14** across every Node service (24.15 broke
  `jest-resolve` under Yarn PnP + ESM); `nvm use` reads `.nvmrc`.
